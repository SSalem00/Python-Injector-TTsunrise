...
def _start_dashboard_bridge(host='127.0.0.1', port=8888):
    from direct.task import Task

    def _make_tee(original, conn):
        class _Tee:
            def write(self, s):
                if s:
                    try: conn.sendall(s)
    ...
    def _pump(task):
        ...
        _bridge_lock.acquire()
        jobs = _bridge_jobs[:]
        del _bridge_jobs[:]
        _bridge_lock.release()
        for conn, body in jobs:
            old_out, old_err = sys.stdout, sys.stderr
            sys.stdout = _make_tee(old_out, conn)
            sys.stderr = _make_tee(old_err, conn)
            _t0 = _time.time()
            try:
                try:
                    exec compile(body, '<dashboard>', 'exec') in globals()
                    sys.stdout.write('\n[done %.2fs]\n' % (_time.time() - _t0))
                except SystemExit:
                    raise
                except:
    ...
    def _handle(conn):
        try:
            ...
            body = ''
            while len(body) < n:
                chunk = conn.recv(min(4096, n - len(body)))
                if not chunk: break
                body += chunk
            _bridge_lock.acquire()
            _bridge_jobs.append((conn, body))
            _bridge_lock.release()
        except:
            try: conn.close()
            except: pass
    ...
    taskMgr.add(_pump, 'dashboard-bridge-pump')

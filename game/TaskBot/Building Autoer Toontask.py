...
class BuildingAutoer:
    ...
    def newStreet(self):
        ...
        if self.startId<9000:
            if int(str(self.startId)[1:])>300:
                if self.shouldChangeDistrict and (self.curDistNum <= len(self.shardIds) or not self.shouldChangeHood):
                    self.startId-=200
                    todo='district'
    ...
    def newHood(self):
        curZoneId = str(base.localAvatar.getZoneId())[0]
        newZoneIdx = self.hoods.index(curZoneId) + 1
        if len(self.hoods) - 1 < newZoneIdx:
            newZoneIdx = 0
        
        if self.hoods[0] == curZoneId:
            self.hoods.append(self.hoods[0])
            del self.hoods[0]
            
        self.startId=int(self.hoods[newZoneIdx]+'102')
        self.teleportBackToStreet()
    ...
    def restock(self):
        ...
        if self.desiredLevel3 in [2, 4]:
            #Only use group lure.
            self.desiredLevel3 -= 1
    ...
    def getThrowTarget(self, battle):
        #Pick a Cog based on our likelihood of killing it. If we can kill multiple targets, the highest level Cog will be targeted.
            throwTarget = None
            lowestLvlSuit = None
            throwDmg = ToontownBattleGlobals.getAvPropDamage(4, self.desiredLevel2, base.localAvatar.experience.getExp(4), base.localAvatar.checkGagBonus(4, self.desiredLevel2))
            for suit in battle.suits:
                suitLvl = suit.getActualLevel()
                if lowestLvlSuit is None or lowestLvlSuit.getActualLevel() > suitLvl:
                    lowestLvlSuit = suit
                    
                #Check for if this Cog will have knockback.
                hasKb = False
                try:
                    hasKb = battle.suits.index(suit) == battle.activeSuits.index(suit) and battle.isSuitLured(suit)
                except ValueError:
                    hasKb = False
                    
                if hasKb and (throwDmg + (throwDmg * 0.5)) >= suit.getHP():
                    if throwTarget is None or throwTarget.getActualLevel() < suitLvl:
                        throwTarget = suit
                        
                elif throwDmg >= suit.getHP():
                    if throwTarget is None or throwTarget.getActualLevel() < suitLvl:
                        throwTarget = suit
                        
            if throwTarget is None:
                #If we can't kill any targets... attack the lowest level Cog for the quickest kill.
                throwTarget = lowestLvlSuit
                if throwTarget is None:
                    #Just attack something.
                    throwTarget = battle.suits[0]
    ...
    def attack(self):
        try:
            for battle in base.cr.doFindAll('battle'):
                throwTarget = self.getThrowTarget(battle)
                allSuitsLured = True
                for suit in battle.suits:
                    if not battle.isSuitLured(suit):
                        allSuitsLured = False
                        
                shouldHeal = base.localAvatar.getTrackAccess()[0] and base.localAvatar.getHp()<(base.localAvatar.getMaxHp()*0.7)
                ableToLure = base.localAvatar.getTrackAccess()[2] and self.desiredLevel3 > 0

                if ableToLure and not allSuitsLured:
                ...
                #We only want to do anything if the Cogs are lured, or if we don't have access to Lure. 
                if allSuitsLured or not ableToLure:
                    if shouldHeal:
                        ...
                        if base.localAvatar.inventory.inventory[4][self.desiredLevel2]>0:
                            battle.sendUpdate('requestAttack', [4, self.desiredLevel2, throwTarget.doId])
                            battle.sendUpdate('movieDone')
                        else:
                            break
        except:

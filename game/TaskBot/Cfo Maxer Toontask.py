...
class OtherFunctions:
    ...
    def getThrowTarget(self, battle):
        #Pick a Cog based on our likelihood of killing it. If we can kill multiple targets, the highest level Cog will be targeted.
            throwTarget = None
            lowestLvlSuit = None
            throwDmg = ToontownBattleGlobals.getAvPropDamage(4, cfoMaxer.restock.desiredLevel3, base.localAvatar.experience.getExp(4), base.localAvatar.checkGagBonus(4, cfoMaxer.restock.desiredLevel3))
            ...
            for battle in base.cr.doFindAll('battle'):
                ...
                if numUnlured > 1 or (self.isHealing and numUnlured != 0):
                    if base.localAvatar.inventory.inventory[cfoMaxer.restock.firstTrack][cfoMaxer.restock.desiredLevel1]>0:
                        battle.sendUpdate('requestAttack', [cfoMaxer.restock.firstTrack, cfoMaxer.restock.desiredLevel1, battle.suits[0].doId])
                        wantThrow = False
                    else:
                        break
